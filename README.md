...
**GitHub setup ping.** GitHub fires an automatic `ping` delivery (`X-GitHub-Event: ping`) the moment a webhook is created, to confirm the endpoint is reachable. It carries no user content — just a `zen` string plus hook/repository metadata — so a GitHub source still verifies its signature and counts it against the rate limit, but discards it with `200 OK` and creates no record, instead of storing it as a blank "Untitled" one. The source's "last hit" timestamp is still refreshed and a `dim` activity event is logged, so a correctly wired source doesn't read as undelivered.
